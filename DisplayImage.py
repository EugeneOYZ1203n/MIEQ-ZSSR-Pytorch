...
def get_args():
    ...
    parser.add_argument('--factors', nargs='+', type=int, help='Scaling factor for img files')
...
if __name__ == "__main__":
    ...
    args.factors = args.factors + [1] * (len(args.imgs)-len(args.factors))

    print("Scaled as: " + ",".join([str(i) for i in args.factors]))
    ...
    for i in range(min(len(args.imgs),len(args.names))):
        ...
        images_resized = [
            image.resize((int(image.width * args.factors[i]), int(image.height * args.factors[i])))
            for image in images_PIL
        ]

        print(f'Added to dictionary under {args.names[i]}: {",".join(images)}')

        image_dict[args.names[i]] = images_resized
